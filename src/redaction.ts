const SECRET_PATTERNS: Array<[RegExp, string]> = [
  ...
  [/(\b[A-Z0-9_]*(?:TOKEN|SECRET|PASSWORD|PASSWD|CREDENTIAL|AUTH)[A-Z0-9_]*\s*=\s*)[^\s'"]+/gi, "$1[REDACTED]"],
  [/((?:api|access|secret|token|password|passwd|pwd)[_-]?key?\s*=\s*)[^\s'"]+/gi, "$1[REDACTED]"],
  [/((?:api|access|secret|token|password|passwd|pwd)[_-]?key?['"]?\s*:\s*['"])[^'"]+/gi, "$1[REDACTED]"]
];
